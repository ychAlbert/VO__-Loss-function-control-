...
class UpdateOperator(nn.Module):
    def __init__(self):
        ...
        self.factor_head_delta = nn.Sequential(
            nn.Linear(64, 32),
            nn.ReLU(),
            nn.Linear(32, 2)  # 预测二维修正值
        )
        self.factor_head_sigma = nn.Sequential(
            nn.Linear(64, 32),
            nn.ReLU(),
            nn.Linear(32, 2),  # 预测置信权重
            nn.Sigmoid()  # 限制在(0,1)范围内
        )
        
        # 束调整层（BA）
        self.ba_layer = nn.Linear(2, 7)  # 调整输出为7维度以匹配relative_pose
...
def train_vo_model(dataset_path, epochs=50, batch_size=32, learning_rate=1e-4):
    """
    训练视觉里程计模型
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=len(dataloader), desc=f'训练轮数 {epoch+1}/{epochs}', unit='batch') as pbar:
            for batch in dataloader:
                ...
                # 计算损失
                loss = criterion(predicted_pose, relative_pose)
